...
def render_ticker_detail(row, show_trend: bool = True):
    ...
    # -----------------------------------------------------------------
    # Blended valuation pillar, DCA allocation tier, expected CAGR
    # -----------------------------------------------------------------
    pillar = row.get("valuation_pillar")
    if pillar and pillar != "P/S growth-gap":
        st.caption(f"🔀 {pillar}")

    e1, e2, e3 = st.columns(3)

    alloc_label = row.get("allocation_classification", "Insufficient data")
    alloc_mult = safe_float(row.get("allocation_multiplier"))
    alloc_color = (
        "#4FBF7A" if alloc_mult is not None and alloc_mult >= 1.5
        else "#8FBF6A" if alloc_mult is not None and alloc_mult >= 1.0
        else "#E0A63A" if alloc_mult is not None and alloc_mult >= 0.75
        else "#E0584F" if alloc_mult is not None
        else DEFAULT_ACCENT
    )
    card(
        e1,
        "DCA allocation tier",
        f"{alloc_label} ({alloc_mult:.2f}x)" if alloc_mult is not None else alloc_label,
        color=alloc_color,
    )

    priority = safe_float(row.get("buy_priority_score"))
    card(e2, "Buy priority score", f"{priority:.0f} / 100" if priority is not None else "n/a")

    expected_cagr = safe_float(row.get("expected_cagr"))
    card(
        e3,
        "Expected CAGR (est.)",
        fmt_signed_pct(expected_cagr) if expected_cagr is not None else "n/a",
        color="#4FBF7A" if expected_cagr is not None and expected_cagr > 0 else DEFAULT_ACCENT,
    )

    if expected_cagr is not None:
        g = safe_float(row.get("expected_cagr_growth_component"))
        i = safe_float(row.get("expected_cagr_income_component"))
        r = safe_float(row.get("expected_cagr_reversion_component"))
        parts = []
        if g is not None:
            parts.append(f"growth {g*100:+.1f}%")
        if i is not None:
            parts.append(f"owner earnings yield {i*100:+.1f}%")
        if r is not None:
            parts.append(f"valuation reversion (3Y annualized) {r*100:+.1f}%")
        st.caption("Expected CAGR ≈ " + " + ".join(parts) + ". Rough estimate, not a forecast guarantee.")
    ...
    if not has_mgmt:
        ...
        persistence_label = row.get("roic_persistence_label", "Insufficient data")
        persistence_score = safe_float(row.get("roic_persistence_score"))
        persistence_color = (
            "#4FBF7A" if persistence_score is not None and persistence_score >= 70
            else "#E0A63A" if persistence_score is not None and persistence_score >= 45
            else "#E0584F" if persistence_score is not None
            else DEFAULT_ACCENT
        )
        card(
            m4,
            "ROIC persistence",
            f"{persistence_label} ({persistence_score:.0f})" if persistence_score is not None else persistence_label,
...
def fetch_adhoc(symbol: str, history_years: int = 5) -> dict:
    ...
    result = compute_valuation(
        ...
        roic_avg_3y=raw.roic_avg_3y,
        roic_trend=raw.roic_trend,
    )
...
tab_overview, tab_payday, tab_detail, tab_search = st.tabs(
    ["Watchlist", "Payday allocation", "Ticker detail", "Ad-hoc search"]
)
...
def render_watchlist_tab():
    ...
    if "anti_bubble_flag" in filtered.columns:
        cols.append("anti_bubble_flag")

    if "buy_priority_score" in filtered.columns:
        cols.append("buy_priority_score")

    if "allocation_classification" in filtered.columns:
        cols.append("allocation_classification")

    view = filtered[[c for c in cols if c in filtered.columns]].copy()

    # Surface the sanity-guard warnings (units/currency mismatches, structural
    # breaks, etc.) that compute_valuation() records in plain_explanation --
    # otherwise a row can show extreme numbers with no visible reason why.
    if "plain_explanation" in filtered.columns:
        view["Data flag"] = filtered["plain_explanation"].apply(
            lambda text: "⚠ Check data" if isinstance(text, str) and text.startswith("⚠") else ""
        )
    ...
    rename_map = {
        ...
        "anti_bubble_flag": "Bubble Read",
        "buy_priority_score": "Buy Priority",
        "allocation_classification": "DCA Tier",
    }

    view = view.rename(columns=rename_map)

    sort_options = [c for c in view.columns if c != "ticker"]
    if "Buy Priority" in sort_options:
        default_sort = "Buy Priority"
    elif "Growth gap %" in sort_options:
        default_sort = "Growth gap %"
    else:
    ...
    column_config = {
        ...
        "Buy Priority": st.column_config.NumberColumn(format="%.1f"),
    }
...
# ---------------------------------------------------------------------------
# Payday allocation tab -- turns the engine from an analysis tool into an
# allocation engine: given a monthly contribution, deploy it top-down by
# Buy Priority Score, sized per-ticker by its DCA allocation multiplier.
# ---------------------------------------------------------------------------

def render_payday_tab():
    if df.empty:
        st.caption("Nothing to show yet -- run the engine to populate the watchlist.")
        return

    if "buy_priority_score" not in df.columns or df["buy_priority_score"].isna().all():
        st.caption(
            "No Buy Priority Score available -- re-run `python -m engine.run` to compute it "
            "(it's calculated cross-sectionally across the whole watchlist)."
        )
        return

    st.caption(
        "Deploys a monthly contribution top-down by Buy Priority Score, sized within each "
        "ticker's target portfolio weight by its valuation-driven DCA multiplier. This is a "
        "sizing aid, not investment advice -- always sanity-check before placing real orders."
    )

    contribution = st.number_input(
        "Monthly contribution ($)", min_value=0.0, value=1000.0, step=50.0
    )

    alloc_df = df.copy()
    alloc_df["allocation_multiplier"] = pd.to_numeric(
        alloc_df.get("allocation_multiplier", 1.0), errors="coerce"
    ).fillna(1.0)
    alloc_df["portfolio_weight"] = pd.to_numeric(
        alloc_df.get("portfolio_weight", 0.0), errors="coerce"
    ).fillna(0.0)

    # Base $ per ticker = contribution split by target portfolio weight (falls
    # back to an even split if weights aren't populated, e.g. ad-hoc-only data).
    if alloc_df["portfolio_weight"].sum() > 0:
        base_weight = alloc_df["portfolio_weight"] / alloc_df["portfolio_weight"].sum()
    else:
        base_weight = pd.Series(1.0 / len(alloc_df), index=alloc_df.index)

    raw_dollars = contribution * base_weight * alloc_df["allocation_multiplier"]

    # Multipliers mean the tiers don't sum back to the contribution amount by
    # construction (that's the point -- cheap names get MORE than their base
    # weight), so rescale the whole set back to the contribution total.
    total_raw = raw_dollars.sum()
    if total_raw > 0:
        alloc_df["Buy $ this payday"] = raw_dollars * (contribution / total_raw)
    else:
        alloc_df["Buy $ this payday"] = 0.0

    view_cols = [
        "ticker", "buy_priority_score", "allocation_classification",
        "allocation_multiplier", "Buy $ this payday",
    ]
    view = alloc_df[[c for c in view_cols if c in alloc_df.columns]].copy()
    ...
    view = view.sort_values("Buy Priority", ascending=False)
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Buy $ this payday": st.column_config.NumberColumn(format="$%.2f"),
        },
    )
