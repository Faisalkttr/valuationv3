...
## v4 changelog (bugfixes + new pillars)

**Bugs fixed:**
- `quality_flag` was silently dropping its `net_margin` unprofitability
  fallback because the call site omitted the argument.
- A cadence-mismatch bug in `fetch.py`: when quarterly income/cashflow
  statements weren't available (common for ADRs and some foreign listings),
  the code fell back to annual statements but kept summing "4 periods" as if
  they were quarters -- overstating TTM free cash flow, owner earnings,
  margins, and capital-allocation figures by ~4x for affected tickers. Fixed
  by having the cadence-aware statement getters return the *actual* cadence
  used, and having every caller sum by that instead of the assumed one.

**New valuation pillars, per the v3 review:**
- **Owner earnings pillar** -- for "mature" tickers (revenue CAGR 3Y < 12%),
  the growth-gap burden score is blended 50/50 with an owner-earnings-yield
  richness score, since revenue growth alone is a weak value driver for
  slow-growing quality compounders.
- **ROIC persistence score** -- ROIC is now tracked across up to 4 annual
  periods (not just the latest one), and a level+trend persistence score
  adjusts the historical-median P/S anchor by up to +/-20%.
- **Dynamic DCA allocation multiplier** -- a 5-tier classification (Deep
  Undervalued 2.0x -> Very Expensive 0.5x) for sizing a periodic contribution,
  surfaced in the new "Payday allocation" dashboard tab.
- **Expected CAGR estimate** -- growth + owner-earnings yield + valuation
  mean-reversion (annualized over 3 years), a rough forward-return estimate.
- **Buy Priority Score** -- a cross-sectional, weighted composite (30%
  valuation / 25% quality / 20% growth durability / 15% owner-earnings yield
  / 10% anti-bubble) ranking every ticker in the watchlist against its peers,
  computed once per run after every ticker is fetched.

`data/history.db`'s schema evolves as new metrics are added; `write_to_history_db()`
now reads whatever's already on disk, unions columns with the new run, and
rewrites the table, so older snapshots survive with `NaN` in the new columns
rather than the run crashing on an `OperationalError`.
