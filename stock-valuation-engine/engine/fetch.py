...
def _roic_for_period(operating_income, pretax, tax, total_debt, equity, cash) -> float | None:
    ...
    invested_capital = (total_debt or 0.0) + (equity or 0.0) - (cash or 0.0)
...
def _roic(tkr: yf.Ticker) -> float | None:
    ...
    try:
        ...
        return _roic_for_period(operating_income, pretax, tax, total_debt, equity, cash)
    except Exception:
        return None
...
def _roic_series(tkr: yf.Ticker, max_periods: int = 4) -> pd.Series:
    ...
    try:
        ...
        op_inc_s = _statement_series(income, "Operating Income", "EBIT")
        if op_inc_s is None or op_inc_s.empty:
            return pd.Series(dtype=float)
        ...
        values: dict = {}
        for period_end in list(op_inc_s.index)[-max_periods:]:
            operating_income = _as_float(op_inc_s.get(period_end))
            pretax = _as_float(pretax_s.get(period_end)) if pretax_s is not None else None
            tax = _as_float(tax_s.get(period_end)) if tax_s is not None else None
            total_debt = _as_float(debt_s.get(period_end)) if debt_s is not None else None
            equity = _as_float(equity_s.get(period_end)) if equity_s is not None else None
            cash = _as_float(cash_s.get(period_end)) if cash_s is not None else None

            roic = _roic_for_period(operating_income, pretax, tax, total_debt, equity, cash)
            if roic is not None:
                values[period_end] = roic
...
def _capital_allocation(tkr: yf.Ticker, cadence: str) -> dict:
    out: dict = {}
    try:
        cashflow, cashflow_cadence = _cashflow_statement_for_cadence(tkr, cadence)
        buybacks = _row_period_sum(cashflow, cashflow_cadence, "Repurchase Of Capital Stock")
        dividends = _row_period_sum(cashflow, cashflow_cadence, "Cash Dividends Paid", "Common Stock Dividend Paid")
        acquisitions = _row_period_sum(cashflow, cashflow_cadence, "Net Business Purchase And Sale", "Purchase Of Business")
...
def _owner_earnings(tkr, market_cap, fx_rate, revenue_ccy, price_ccy, cadence) -> dict:
    out: dict = {}
    try:
        income, income_cadence = _income_statement_for_cadence(tkr, cadence)
        cashflow, cashflow_cadence = _cashflow_statement_for_cadence(tkr, cadence)

        net_income = _row_period_sum(income, income_cadence, "Net Income", "Net Income Common Stockholders")
        op_cf = _row_period_sum(cashflow, cashflow_cadence, "Operating Cash Flow",
                                "Cash Flow From Continuing Operating Activities")
        da = _row_period_sum(cashflow, cashflow_cadence, "Depreciation And Amortization",
                             "Depreciation & Amortization", "Depreciation Amortization Depletion",
                             "Depreciation, Amortization & Depletion", "D&A")
        if da is None:
            da = _row_period_sum(income, income_cadence, "Depreciation And Amortization",
                                 "Depreciation & Amortization", "Depreciation Amortization Depletion",
                                 "Depreciation, Amortization & Depletion", "D&A")

        capex = _row_period_sum(cashflow, cashflow_cadence, "Capital Expenditure")
...
def _fundamentals(tkr: yf.Ticker, revenue_ttm: float, cadence: str) -> dict:
    ...
    try:
        ...
        gross_profit = _row_period_sum(income, income_cadence, "Gross Profit")
        operating_income = _row_period_sum(
            income, income_cadence,
    ...
    try:
        cashflow, cashflow_cadence = _cashflow_statement_for_cadence(tkr, cadence)
        fcf = _row_period_sum(cashflow, cashflow_cadence, "Free Cash Flow")
        if fcf is None:
            op_cf = _row_period_sum(cashflow, cashflow_cadence, "Operating Cash Flow",
                                    "Cash Flow From Continuing Operating Activities")
            capex = _row_period_sum(cashflow, cashflow_cadence, "Capital Expenditure")
...
def fetch_ticker(symbol: str, history_years: int = 5) -> RawTickerData:
    ...
    roic_series = _roic_series(tkr)
    if not roic_series.empty:
        extras["roic"] = float(roic_series.iloc[-1])
        extras["roic_avg_3y"] = float(roic_series.tail(3).mean())
        if len(roic_series) >= 2:
            extras["roic_trend"] = float(roic_series.iloc[-1] - roic_series.iloc[0])
